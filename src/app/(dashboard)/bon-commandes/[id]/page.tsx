    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <Card><CardHeader><CardTitle>Montants</CardTitle></CardHeader>
                    <CardContent className="space-y-2">
                        <div><span className="text-sm text-muted-foreground">{bc.inclure_tva ? "HT" : "Montant Total"}</span><p>{Number(bc.montant_ht).toFixed(2)} MAD</p></div>
                        {bc.inclure_tva && (
                            <div><span className="text-sm text-muted-foreground">TVA</span><p>{Number(bc.montant_tva).toFixed(2)} MAD</p></div>
                        )}
                        <div><span className="text-sm text-muted-foreground">Net à Payer</span><p className="text-lg font-bold">{Number(bc.montant_ttc).toFixed(2)} MAD</p></div>
                    </CardContent>
                </Card>
            </div>
            <Card><CardHeader><CardTitle>Lignes</CardTitle></CardHeader><CardContent><div className="rounded-md border"><Table><TableHeader><TableRow><TableHead>Désignation</TableHead><TableHead className="w-[100px]">Qté</TableHead><TableHead className="w-[120px]">Prix unit.</TableHead>{bc.inclure_tva && <TableHead className="w-[80px]">TVA</TableHead>}<TableHead className="w-[120px] text-right">{bc.inclure_tva ? "Montant HT" : "Montant Total"}</TableHead></TableRow></TableHeader><TableBody>{bc.lignes?.map((l) => (<TableRow key={l.id}><TableCell>{l.designation}</TableCell><TableCell>{l.quantite}</TableCell><TableCell>{Number(l.prix_unitaire).toFixed(2)} MAD</TableCell>{bc.inclure_tva && <TableCell>{l.tva}%</TableCell>}<TableCell className="text-right font-medium">{Number(l.montant_ht).toFixed(2)} MAD</TableCell></TableRow>))}</TableBody><TableFooter><TableRow><TableCell colSpan={bc.inclure_tva ? 4 : 3} className="text-right font-medium">{bc.inclure_tva ? "Total HT" : "Montant Total"}</TableCell><TableCell className="text-right font-bold">{Number(bc.montant_ht).toFixed(2)} MAD</TableCell></TableRow>{bc.inclure_tva && <TableRow><TableCell colSpan={4} className="text-right font-medium">TVA</TableCell><TableCell className="text-right font-bold">{Number(bc.montant_tva).toFixed(2)} MAD</TableCell></TableRow>}<TableRow><TableCell colSpan={bc.inclure_tva ? 4 : 3} className="text-right text-lg font-medium">Net à Payer</TableCell><TableCell className="text-right font-bold text-lg">{Number(bc.montant_ttc).toFixed(2)} MAD</TableCell></TableRow></TableFooter></Table></div></CardContent></Card>
