            ...
            <div className="grid gap-4 md:grid-cols-3">
                ...
                <Card><CardHeader><CardTitle>Montants</CardTitle></CardHeader>
                    <CardContent className="space-y-2">
                        <div><span className="text-sm text-muted-foreground">{ba.inclure_tva ? "HT" : "Montant Total"}</span><p>{Number(ba.montant_ht).toFixed(2)} MAD</p></div>
                        {ba.inclure_tva && (
                            <div><span className="text-sm text-muted-foreground">TVA</span><p>{Number(ba.montant_tva).toFixed(2)} MAD</p></div>
                        )}
                        <div><span className="text-sm text-muted-foreground">Net à Payer</span><p className="text-lg font-bold">{Number(ba.montant_ttc).toFixed(2)} MAD</p></div>
                    </CardContent>
                </Card>
            </div>
            <Card><CardHeader><CardTitle>Lignes</CardTitle></CardHeader><CardContent><div className="rounded-md border"><Table><TableHeader><TableRow><TableHead>Désignation</TableHead><TableHead className="w-[100px]">Qté</TableHead><TableHead className="w-[120px]">Prix unit.</TableHead>{ba.inclure_tva && <TableHead className="w-[80px]">TVA</TableHead>}<TableHead className="w-[120px] text-right">{ba.inclure_tva ? "Montant HT" : "Montant Total"}</TableHead></TableRow></TableHeader><TableBody>{ba.lignes?.map((l) => (<TableRow key={l.id}><TableCell>{l.designation}</TableCell><TableCell>{l.quantite}</TableCell><TableCell>{Number(l.prix_unitaire).toFixed(2)} MAD</TableCell>{ba.inclure_tva && <TableCell>{l.tva}%</TableCell>}<TableCell className="text-right font-medium">{Number(l.montant_ht).toFixed(2)} MAD</TableCell></TableRow>))}</TableBody><TableFooter><TableRow><TableCell colSpan={ba.inclure_tva ? 4 : 3} className="text-right font-medium">{ba.inclure_tva ? "Total HT" : "Montant Total"}</TableCell><TableCell className="text-right font-bold">{Number(ba.montant_ht).toFixed(2)} MAD</TableCell></TableRow>{ba.inclure_tva && <TableRow><TableCell colSpan={4} className="text-right font-medium">TVA</TableCell><TableCell className="text-right font-bold">{Number(ba.montant_tva).toFixed(2)} MAD</TableCell></TableRow>}<TableRow><TableCell colSpan={ba.inclure_tva ? 4 : 3} className="text-right text-lg font-medium">Net à Payer</TableCell><TableCell className="text-right font-bold text-lg">{Number(ba.montant_ttc).toFixed(2)} MAD</TableCell></TableRow></TableFooter></Table></div></CardContent></Card>
